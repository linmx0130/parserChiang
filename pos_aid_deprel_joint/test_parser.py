...
ctx = mx.gpu(0)
parserModel = ParserModel(len(word_map), len(pos_map), config.EMBED_SIZE, config.POS_EMBED_SIZE, config.NUM_HIDDEN, len(deprel_map))
...
logging.info("Evaling: Pred acc={:.6} Model acc={:.6} UAS={:.6} LAS={:.6}".format(acc/total_tags, model_acc/model_total_tags, uas/total_tokens, las/total_tokens))
logging.shutdown()
